...
//set controller into autoclave model
		...
		case RUNNING:
		  ...
		  //Zeige Errormeldung, schließe Protokoll ab falls Error von Mikrocontroller gemeldet wurde
		...
		case PROGRAM_FINISHED:
			...
			if(Autoclave.getInstance().getCurrentProgramCounter() < Autoclave.getInstance().getProgramsInRowTotal()){
				...
				if(Autoclave.getInstance().getData().isDoorClosed() == false){//autoklav wurde geöffnet
					Autoclave.getInstance().setState(AutoclaveState.NOT_RUNNING);
				}
			}
		...
		//falls jemand zuhört, wird ihm die aktuelle Fehlerliste geschickt.
	...
	private void updateErrorList(){
		...
		case PREPARE_TO_RUN:
			...
			if(Autoclave.getInstance().getData().isDoorClosed() == false){
				errorList.add(new Error("Please close the door",
	 ...
	 * 
	 * @param errorMessage
	 *            Message, which will be added at the end of the protocol
	 * 
	 */
